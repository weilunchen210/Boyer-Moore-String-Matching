...
public class BoyerMoore {
    ...
    private static int[] preprocessGoodSuffixShift(String pattern) {
        ...
        Arrays.fill(shift, 0); // Initialize all elements in shift array to 0

        // Preprocess the strong and partial suffix shift table
        preprocessStrongSuffix(shift, borderPos, pattern);
        preprocessPartialSuffix(shift, borderPos, pattern);
        return shift;
    }
    ...
    private static void preprocessPartialSuffix(int[] shift, int[] borderPos, String pattern) {
        int j = borderPos[0]; // Initialize j with the position of the first character with the same suffix

        for (int i = 0; i <= pattern.length(); i++) {
            if (shift[i] == 0) {
                // If shift[i] is not already assigned, assign the value of j to it
                shift[i] = j;
            }
            if (i == j) {
                j = borderPos[j]; // Move j to the position of the next character with the same suffix
