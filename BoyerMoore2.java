...
public class BoyerMoore2 {
    ...
    public static List<Integer> boyerMoore(String text, String pattern) {
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (j < 0) {
                ...
                int badCharShift = Math.max(1, j - badCharacter[text.charAt(j + i)]);
    ...
    private static int[] preprocessBadCharacter(String pattern) {
        ...
        // Assigns value to the rightmost character based on their position in the pattern
        for (int i = 0; i < pattern.length() - 1 ; i++) {
            char currentChar = pattern.charAt(i);
            badCharacter[currentChar] = i;
        }
        return badCharacter;
    }
